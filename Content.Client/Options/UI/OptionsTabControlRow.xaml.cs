...
public sealed partial class OptionsTabControlRow : Control
{
    ...
    private ValueList<DangerousOptionChange> _dangerousChanges;
    private OptionsDangerousConfirmWindow? _dangerousConfirmWindow;
    ...
    public T AddOption<T>(T option, bool dangerous = false) where T : BaseOption
    {
    ...
    public OptionCheckboxCVar AddOptionCheckBox(
        CVarDef<bool> cVar,
        CheckBox checkBox,
        bool invert = false,
        bool dangerous = false)
    {
        return AddOption(new OptionCheckboxCVar(this, _cfg, cVar, checkBox, invert), dangerous);
    }
    ...
    public OptionDropDownCVar<T> AddOptionDropDown<T>(
        ...
        where T : notnull
    {
        return AddOption(new OptionDropDownCVar<T>(this, _cfg, cVar, dropDown, options), dangerous);
    }
    ...
    /// </remarks>
    public void ValueChanged(BaseOption option)
    {
        if (option.Dangerous)
        {
            ShowDangerousConfirmation(option.ApplyValueForConfirmation());
            UpdateButtonState();
            return;
        }
    ...
    /// <summary>
    /// Reverts all unsaved changes back to the values captured when the menu was opened or last saved.
    /// </summary>
    ...
    private void ShowDangerousConfirmation(DangerousOptionChange change)
    {
        ...
        {
            window.Reset(ConfirmDangerousChanges, RejectDangerousChanges);
            window.MoveToFront();
            return;
        }

        _dangerousConfirmWindow = new OptionsDangerousConfirmWindow(
    ...
    {
        foreach (var change in _dangerousChanges)
        {
            change.Confirm();
        }

        _dangerousChanges.Clear();
        UpdateButtonState();
    }

    private void RejectDangerousChanges()
    {
        for (var i = _dangerousChanges.Count - 1; i >= 0; i--)
        {
            _dangerousChanges[i].Reject();
        }
...
{
    /// <summary>
    /// Indicates change requires confirmation and shows a timer before applying.
    /// </summary>
    ...
    /// <summary>
    /// Applies the current UI state to the backing store immediately.
    /// </summary>
    public abstract void ApplyValue();

    /// <summary>
    /// Applies a dangerous option change and returns callbacks for confirming or rejecting that specific change.
    /// </summary>
    ...
    /// <summary>
    /// Reverts the backing store and UI to the value captured when the menu was opened or last saved.
    /// </summary>
    public abstract void RevertValue();

    /// <summary>
    /// Reverts only the most recent applied value. Used when a dangerous option change is rejected.
    /// </summary>
    public abstract void RevertRecentChange();

    /// <summary>
    /// Accepts the current value for dangerous-option confirmation purposes.
    /// </summary>
    public virtual void AcceptCurrentValue()
    {
    }
    ...
    where TValue : notnull
{
    ...
    private TValue _previousAppliedValue = default!;
    ...
    {
        var value = _cfg.GetCVar(_cVar);
        _originalValue = value;
        _previousAppliedValue = value;
    ...
    {
        var current = _cfg.GetCVar(_cVar);
        if (IsValueEqual(Value, current))
            return;

        _previousAppliedValue = current;
        _cfg.SetCVar(_cVar, Value);
    }

    public override DangerousOptionChange ApplyValueForConfirmation()
    {
        var previous = _cfg.GetCVar(_cVar);
        var value = Value;
    ...
    {
        SetValueSilently(_originalValue);
        _cfg.SetCVar(_cVar, _originalValue);
        _previousAppliedValue = _originalValue;
        ImmediateValueChanged?.Invoke(Value);
    }

    public override void RevertRecentChange()
    {
        SetValueSilently(_previousAppliedValue);
        _cfg.SetCVar(_cVar, _previousAppliedValue);
        ImmediateValueChanged?.Invoke(Value);
    }

    public override bool IsModified()
    {
        return !IsValueEqual(Value, _originalValue);
    }
...
public sealed class OptionsDangerousConfirmWindow : DefaultWindow
{
    private const float ConfirmSeconds = 6.7f;
    ...
    public OptionsDangerousConfirmWindow(Action confirm, Action reject)
    {
        _confirm = confirm;
        _reject = reject;

        Title = Loc.GetString("ui-options-dangerous-confirm-title");
        MinSize = new Vector2(360, 120);
        ...
        {
            Text = Loc.GetString("ui-options-dangerous-confirm-message"),
        ...
        {
            Text = Loc.GetString("ui-options-dangerous-confirm-reject"),
            StyleClasses = { "negative" },
        };
        rejectButton.OnPressed += _ => Reject();
        buttons.AddChild(rejectButton);

        var confirmButton = new Button
        {
            Text = Loc.GetString("ui-options-dangerous-confirm-confirm"),
    ...
    public void Reset(Action confirm, Action reject)
    {
        _confirm = confirm;
        _reject = reject;
        _elapsed = 0f;
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        _progressBar.Value = Math.Clamp(_elapsed / ConfirmSeconds, 0, 1);

        if (_elapsed >= ConfirmSeconds)
            Reject(deferClose: true);
    }
    ...
    private void Reject(bool deferClose = false)
    {
        Finish(_reject, deferClose);
    }

    private void Finish(Action action, bool deferClose = false)
    {
        if (_finished)
            return;
