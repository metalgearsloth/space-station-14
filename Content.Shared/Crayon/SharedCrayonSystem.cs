...
public abstract class SharedCrayonSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CrayonComponent, UseInHandEvent>(OnCrayonUse, before: new[] { typeof(SharedFoodSystem) });
        SubscribeLocalEvent<CrayonComponent, AfterInteractEvent>(OnCrayonAfterInteract, after: new[] { typeof(SharedFoodSystem) });
        SubscribeLocalEvent<CrayonComponent, DroppedEvent>(OnCrayonDropped);
        SubscribeLocalEvent<CrayonComponent, MapInitEvent>(OnCrayonMapInit);
        SubscribeLocalEvent<CrayonComponent, CrayonSelectMessage>(OnCrayonBoundUI);
        SubscribeLocalEvent<CrayonComponent, CrayonColorMessage>(OnCrayonBoundUIColor);
    }

    private void OnCrayonMapInit(Entity<CrayonComponent> ent, ref MapInitEvent args)
    {
        ent.Comp.Charges = ent.Comp.Capacity;
    ...
    private void OnCrayonAfterInteract(EntityUid uid, CrayonComponent component, AfterInteractEvent args)
    {
        ...
        if (component.Charges <= 0)
        {
            if (component.DeleteEmpty)
                ...
                _popup.PopupClient(Loc.GetString("crayon-interact-not-enough-left-text"), uid, args.User);
        ...
        {
            _popup.PopupClient(Loc.GetString("crayon-interact-invalid-location"), uid, args.User);
            args.Handled = true;
            return;
        }
        ...
        if (component.UseSound != null)
            _audio.PlayPredicted(component.UseSound, uid, args.User, AudioParams.Default.WithVariation(0.125f));
    ...
    protected virtual void UseUpCrayon(EntityUid uid, EntityUid user)
    {
        _popup.PopupClient(Loc.GetString("crayon-interact-used-up-text", ("owner", uid)), user, user);
    }
}
